...
First-person hand rendering is opt-in. `PPlayerFirstPersonSettings.DISABLED` is the default. When a definition with `firstPerson(PPlayerFirstPersonSettings.ENABLED)` contributes for the local player, PulseLib wraps Minecraft's `submitHandsWithItems` pass and resolves each arm and item from the animation presentation. Channels without a sampled transform retain the corresponding vanilla submission, including swing, equip, use, map, and item-model transforms.
